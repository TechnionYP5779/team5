...
public interface idiomatic {
	...
	Trigger eval = new Trigger() {
		@Override
		public <@Nullable T> T eval(final Supplier<@Nullable T> ¢) {
			return ¢.get();
		}
	};
	/** an ignoring trigger */
	...
	Trigger ignore = new Trigger() {
		@Override
		@Nullable
		public <@Nullable T> T eval(final Supplier<T> ____) {
			return null;
		}
	};
	...
	@Nullable
	static <@Nullable T> T incase(final boolean condition, final T t) {
