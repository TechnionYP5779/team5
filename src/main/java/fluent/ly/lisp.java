...
public interface lisp {
	@Nullable
	static <T> List<T> chop(final @NotNull List<T> ¢) {
		if (¢.isEmpty())
			return null;
		¢.remove(0);
		return ¢;
	...
	@NotNull
	static <T> List<T> cons(final T first, final @NotNull List<T> rest) {
		rest.add(0, first);
		return rest;
	}

	@Nullable
	static <T> T next(final int i, final @Nullable List<@Nullable T> ts) {
	...
	@Nullable
	static <T> List<T> replace(final @Nullable List<T> ts, final T element, final int index) {
	...
	@Contract("null, _ -> null")
	@Nullable
	static <T> List<T> replaceLast(final @NotNull List<T> ts, final T element) {
		return replace(ts, element, ts.size() - 1);
	}

	@NotNull
	static <T> Iterable<T> rest2(final @NotNull Iterable<T> ¢) {
