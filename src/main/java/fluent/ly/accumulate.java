...
public interface accumulate<T, C extends Collection<T>> {
	...
	static <T, C extends Collection<T>> @NotNull accumulate<T, C> to(final @NotNull C c) {
		return new accumulate<>() {
			@Override
			@NotNull
			public accumulate<T, C> add(final @Nullable T ¢) {
				if (¢ == null)
					return this;
				c.add(¢);
				return this;
	 ...
	 */
	@NotNull
	default accumulate<T, C> add(final @NotNull Iterable<? extends @Nullable T> ts) {
	 ...
	 */
	@NotNull
	accumulate<T, C> add(@Nullable T t);
	 ...
	 */
	@NotNull
	default accumulate<T, C> addAll(@SuppressWarnings("unchecked") final @Nullable Iterable<? extends T>... tss) {
		if (tss != null)
