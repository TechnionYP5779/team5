...
public class rangeOrig {
  public class AfterTo extends RangeIterator<AfterTo> {
    ...
    public Iterable<Integer> infinite() {
      return rangeOrig.infiniteFrom(from, step);
    }
  ...
  private static rangeOrig makeFrom(final int ¢) {
    return new rangeOrig() {
      {
        from = ¢;
      }
    };
  ...
  private static rangeOrig makeTo(final int ¢) {
    return new rangeOrig() {
      {
        to = ¢;
      }
