...
public enum prune {
	;
	@NotNull
	private static String @NotNull [] asArrray(final @NotNull List<String> $) {
		return cantBeNull($.toArray(new String[0]));
	}

	@NotNull
	public static <T, C extends Collection<T>> C nulls(final @NotNull C $) {
	...
	@NotNull
	public static <T> List<T> nulls(final @NotNull Iterable<T> ts) {
	...
	@SafeVarargs
	public static <T> @NotNull String @NotNull [] whites(final @Nullable T @Nullable... ts) {
