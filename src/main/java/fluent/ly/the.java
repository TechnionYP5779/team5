...
public interface the {
	...
	@Contract("null -> null")
	@Nullable
	static <T> T headOf(final @Nullable List<T> ¢) {
	...
	@Contract(pure = true)
	static char ith(final @NotNull String s, final int i) {
		return s.charAt(i);
	}

	@Contract("null -> null")
	@Nullable
	static <@Nullable T> T lastOf(final @Nullable List<@Nullable T> ¢) {
	...
	@Nullable
	static <T> T onlyOneOf(final @Nullable List<T> ¢) {
		return ¢ == null || ¢.size() != 1 ? null : headOf(¢);
	}

	@Contract("null -> null")
	@Nullable
	static <T> T secondOf(final @Nullable List<T> ¢) {
