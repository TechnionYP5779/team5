 ...
 * import il.org.spartan.statistics.*; import il.org.spartan.utils.*; import
 * static il.org.spartan.utils.Box.*; import java.util.*; import
...
public abstract class CSVLine extends AbstractStringProperties implements Cloneable {
	...
	public final String asKeyValuePairs() {
		return Separate.by((F<@NotNull Entry<@NotNull String, @NotNull String>>) λ -> λ.getKey() + "=" + λ.getValue(),
				entries(), ", ");
	}
	...
	public <T> T extract(final T $) {
		for (@NotNull
		final Entry<String, String> ¢ : External.Introspector.toOrderedMap($).entrySet())
			put(¢.getKey(), ¢.getValue());
		return $;
	}
	 ...
	 * @return this
	 */
	public CSVLine put(final @NotNull String key, final double value, final @NotNull String format,
			@NotNull final FormatSpecifier... ss) {
		aggregator.record(key, value, ss);
	 ...
	 * @return this
	 */
	public CSVLine put(final @NotNull String key, final int value, final @NotNull String format,
			@NotNull final FormatSpecifier... ss) {
		aggregator.record(key, value, ss);
		___.sure(ss.length == 0 || aggregating());
		return put(key, String.format(format, box(value)));
	...
	@Override
	@NotNull
	public final CSVLine put(final @NotNull String key, final @NotNull String value) {
		map.put(key, value);
		return this;
	}
