...
public interface Utils {
	...
	@NotNull
	static <T, C extends Collection<T>> C add(final @NotNull C $, final Iterable<? extends T> ts) {
		for (final T ¢ : ts)
			if (¢ != null)
				$.add(¢);
		return $;
	...
	@SafeVarargs
	static <T, C extends Collection<T>> @NotNull C add(final @NotNull C $, final T @NotNull... ts) {
		for (final T ¢ : ts)
			if (¢ != null)
				$.add(¢);
		return $;
	...
	@SafeVarargs
	@NotNull
	static <T, C extends Collection<T>> C addAll(final @NotNull C $, final Collection<? extends T> @NotNull... tss) {
	...
	@SafeVarargs
	@NotNull
	static <T, C extends Collection<T>> C addAll(final @NotNull C $, final Iterable<? extends T> @NotNull... tss) {
	...
	@SafeVarargs
	@NotNull
	static <T, C extends Collection<T>> C addAll(final @NotNull C $, final T... ts) {
		if (ts != null)
			for (final T ¢ : ts)
				if (¢ != null)
					add($, ¢);
	 ...
	 */
	@SafeVarargs
	static boolean intIsIn(final int candidate, final int... is) {
		for (final int ¢ : is)
			if (¢ == candidate)
				return true;
		return false;
	...
	class Applicator<F, T> {
		...
		@SafeVarargs
		public final Iterable<T> to(final @NotNull F... fs) {
	...
	class FoundHandleForT<T> {
		 ...
		 */
		@SafeVarargs
		public final boolean in(final T... ts) {
			if (ts == null)
				return false;
			for (final T ¢ : ts)
		...
		public static class FoundHandleForInt {
			 ...
			 */
			@SafeVarargs
			public final boolean in(final int... is) {
				for (final int ¢ : is)
					if (¢ == candidate)
						return true;
				return false;
