...
public class BoxTest {
	...
	public void testBooleanBox() {
		...
		@NotNull
		final Boolean[] arrBoxed = box.box(arr);
		for (int ¢ = 0; ¢ < 4; ++¢)
	...
	public void testByteBox() {
		...
		@NotNull
		final Byte[] arrBoxed = box.box(arr);
		azzert.that(unbox.it(arrBoxed), is(arr));
	}

	@Test
	public void testCharBox() {
	...
	public void testFloatBox() {
		...
		for (int ¢ = 0; ¢ < 10; ++¢)
			assert box.it(¢ + diff + (float) 3.14).compareTo(FloatArray[¢]) >= 0
					&& box.it(¢ + (float) 3.14 - diff).compareTo(FloatArray[¢]) <= 0;
		final Float Float2Array[] = box.box(fArray);
		for (int ¢ = 0; ¢ < 10; ++¢)
			assert box.it(¢ + diff + (float) 3.14).compareTo(Float2Array[¢]) >= 0
					&& box.it(¢ + (float) 3.14 - diff).compareTo(FloatArray[¢]) <= 0;
	}
	...
	public void testItFloatBox() {
		...
		for (int ¢ = 0; ¢ < 10; ++¢)
			assert unbox.it(FloatArray[¢]) <= ¢ + diff + (float) 3.14
					&& unbox.it(FloatArray[¢]) >= ¢ + (float) 3.14 - diff;
	}

	@Test
	public void testItIntegerBox() {
		Integer b = box.it(3);
		azzert.that(b, is(box.it(3)));
		b = box.it(-2);
		azzert.that(b, is(box.it(-2)));
