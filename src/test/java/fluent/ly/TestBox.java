...
public class TestBox {
  @Test  public void testBooleanBox() {
    ...
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(BooleanArray[¢], azzert.is(box.box(¢ % 2 == 0)));
  }

  @Test public void testByteBox() {
    Byte b = box.box((byte) 6);
    azzert.that(b, azzert.is(box.box((byte) 6)));
    b = box.box((byte) 0);
    azzert.that(b, azzert.is(box.box((byte) 0)));
    b = box.box((byte) -5);
    azzert.that(b, azzert.is(box.box((byte) -5)));
    final byte bArray[] = new byte[10];
    for (int ¢ = 0; ¢ < 10; ++¢)
      bArray[¢] = (byte) ¢;
    final Byte ByteArray[] = box.box(bArray);
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(ByteArray[¢], azzert.is(box.box((byte) ¢)));
  }

  @Test public void testCharBox() {
    Character b = box.box('a');
    azzert.that(b, azzert.is(box.box('a')));
    b = box.box('b');
    azzert.that(b, azzert.is(box.box('b')));
    final char cArray[] = new char[10];
    for (int ¢ = 0; ¢ < 10; ++¢)
      cArray[¢] = (char) (¢ + 'a');
    final Character CharArray[] = box.box(cArray);
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(CharArray[¢], azzert.is(box.box((char) (¢ + 'a'))));
  }

  @Test public void testDoubleBox() {
    Double b = box.box(3.14);
    azzert.that(b, azzert.is(box.box(3.14)));
    b = box.box(2.71);
    azzert.that(b, azzert.is(box.box(2.71)));
    final double dArray[] = new double[10];
    for (int ¢ = 0; ¢ < 10; ++¢)
      dArray[¢] = ¢ + 3.14;
    final Double DoubleArray[] = box.box(dArray);
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(DoubleArray[¢], azzert.is(box.box(¢ + 3.14)));
  }
  ...
  @Test public void testIntegerBox() {
    Integer b = box.box(3);
    azzert.that(b, azzert.is(box.box(3)));
    b = box.box(-2);
    azzert.that(b, azzert.is(box.box(-2)));
    final int iArray[] = new int[10];
    for (int ¢ = 0; ¢ < 10; ++¢)
      iArray[¢] = ¢;
    final Integer IntegerArray[] = box.box(iArray);
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(IntegerArray[¢], azzert.is(box.box(¢)));
  }

  @Test public void testLongBox() {
    final long offset = (long) 1e7;
    Long b = box.box(3 * offset);
    azzert.that(b, azzert.is(box.box(3 * offset)));
    b = box.box(-3 * offset);
    azzert.that(b, azzert.is(box.box(-3 * offset)));
    ...
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(LongArray[¢], azzert.is(box.box(¢ * offset)));
  }

  @Test public void testShortBox() {
    Short b = box.box((short) 3);
    azzert.that(b, azzert.is(box.box((short) 3)));
    b = box.box((short) -2);
    azzert.that(b, azzert.is(box.box((short) -2)));
    final short sArray[] = new short[10];
    for (int ¢ = 0; ¢ < 10; ++¢)
      sArray[¢] = (short) ¢;
    final Short ShortArray[] = box.box(sArray);
    for (int ¢ = 0; ¢ < 10; ++¢)
      azzert.that(ShortArray[¢], azzert.is(box.box((short) ¢)));
  }
